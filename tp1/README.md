...
**Nota sobre el proceso:** estos tres últimos cambios (base64 en vez de fetch, un solo archivo en vez de carpeta, y el fix del estiramiento) los resolví directamente en el código, no a través de un prompt a Gemini. El prompt equivalente para que Gemini genere la misma estructura (lectura del PDF desde una constante en base64 en vez de por URL, con esa constante al final del script) está documentado en `prompts.md` para dejar el proceso completo, aunque en este caso el código real que se usó en la entrega salió de mi intervención directa y no de la respuesta de Gemini.
...
**El libro bloqueado abría una pantalla en blanco.** Esto pasó solo durante las pruebas con otra IA (Claude), en un entorno separado llamado Claude Artifact, no en Gemini ni en el código final. Claude probó ahí un PDF embebido en base64 dentro de un <iframe>, y ese entorno de prueba tiene restricciones de seguridad que bloquean mostrar un PDF así — la pantalla quedaba en blanco. La solución fue cambiar el iframe por otra forma de mostrar el PDF: la librería pdf.js, que dibuja cada página en un <canvas>. Ese cambio no fue en vano: terminó siendo la misma técnica que usa el index.html final.
